    ...
    <div className="bg-[#f6f3ef] text-[#28302f]">
      <section className="relative overflow-hidden bg-[#41342e] text-white">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-4 pb-16 pt-14 sm:px-6 lg:grid-cols-[1.05fr_.95fr] lg:px-8 lg:pb-24 lg:pt-20">
          <div className="max-w-3xl self-center">
            ...
            <h1 className="apex-hero-enter apex-hero-enter-delay-1 font-serif-display text-4xl font-bold leading-[1.06] text-white sm:text-5xl lg:text-7xl">Protecting what matters most: your family, your future, your peace of mind.</h1>
          ...
          <div className="apex-hero-panel self-end border border-[#d6a84d]/40 bg-[#41342e]/95 p-6 shadow-2xl backdrop-blur-sm sm:p-8">
            <div className="flex items-start justify-between gap-4 border-b border-white/15 pb-5"><div><p className="text-xs font-bold uppercase tracking-[.18em] text-[#f4d994]">Your next right step</p><h2 className="mt-2 text-2xl font-bold text-white">A clear plan starts here.</h2></div><Scale className="h-8 w-8 text-[#d6a84d]" /></div>
            ...
            <p className="mt-4 flex items-center gap-2 text-xs text-[#9fb4b0]"><LockKeyhole className="h-3.5 w-3.5" /> Your information is treated as confidential.</p>
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8" id="practice-areas">
        ...
        <div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-5">
          {FAMILY_PRACTICE_AREAS.map((pillar, index) => { const Icon = pillarIcons[index]; return <article key={pillar.id} data-reveal className="apex-reveal apex-card flex min-h-[330px] flex-col border border-[#d9d5ce] bg-white p-6 transition hover:-translate-y-1 hover:border-[#d6a84d] hover:bg-[#fffdf9]" style={{ transitionDelay: `${index * 80}ms` }}><Icon className="h-7 w-7 text-[#a87920]" /><h3 className="mt-8 text-xl font-bold text-[#173b3b]">{pillar.title}</h3><p className="mt-3 flex-1 text-sm leading-6 text-slate-600">{pillar.shortDesc}</p><Link to={`/practice-areas/${pillar.id}`} className="apex-card-arrow mt-6 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-[#173b3b]">Explore practice <ArrowRight className="h-4 w-4 text-[#a87920]" /></Link></article>; })}
        </div>
      </section>
